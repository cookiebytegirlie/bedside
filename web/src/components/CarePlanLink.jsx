...
export default function CarePlanLink({ to, fileName, lastUpdated }) {
  return (
    <Link
      to={to}
      className="flex items-center gap-3 rounded-[8px] bg-white p-4 shadow-card active:scale-[0.98]"
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block text-base font-bold text-ink">Full Care Plan · {fileName}</span>
        <span className="block text-sm font-semibold text-muted">Last updated {formatLastUpdated(lastUpdated)}</span>
      </span>
      <ChevronRightIcon width={17} height={17} strokeWidth={2.5} className="shrink-0 text-mist" />
    </Link>
  )
}
