...
const URGENCY = {
  green: { label: 'Routine', bg: 'bg-routine-tint', fg: 'text-routine-fg', Icon: CheckIcon },
  yellow: { label: 'Keep an eye on', bg: 'bg-watch-tint', fg: 'text-watch-fg', Icon: AlertTriangleIcon },
  red: { label: 'Needs attention', bg: 'bg-attention-tint', fg: 'text-attention-fg', Icon: BellIcon },
}
...
function UrgencyBadge({ urgency }) {
  ...
  const { Icon } = u
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[13px] font-semibold tracking-tight ${u.bg} ${u.fg}`}>
...
function WorkedMark({ worked }) {
  if (worked === 'yes')
    return (
      <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-routine-tint text-routine-fg">
        <CheckIcon width={12} height={12} strokeWidth={3} />
      </span>
    )
  if (worked === 'no')
    return (
      <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-attention-tint text-attention-fg">
        <XIcon width={12} height={12} strokeWidth={3} />
      </span>
    )
  return (
    <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-watch-tint text-[11px] font-bold text-watch-fg">
      ?
    </span>
  )
}
...
export default function AiResultCard({ transcript, result, seeMeds = true }) {
  ...
  return (
    <div className={`rounded-card border bg-white p-4 ${lowConfidence ? 'border-watch-solid/40 ring-1 ring-watch-solid/30' : 'border-line'}`}>
      {/* Low-confidence: frame the whole card as unconfirmed rather than fact */}
      {lowConfidence && (
        <div className="mb-3 flex items-start gap-2 rounded-card bg-watch-tint p-3">
      ...
      <p className="text-[11px] font-semibold uppercase tracking-wide text-faint">What you said</p>
      ...
      <div className="mt-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-faint">Summary</p>
          <p className="mt-0.5 text-[15px] font-semibold leading-snug text-ink">{result.summary}</p>
        </div>
            ...
            <ul className="space-y-1.5">
              {result.medications.map((m, i) => (
                <li key={i} className="flex items-center gap-2 text-[14px] font-medium text-ink">
                  <PillIcon width={15} height={15} strokeWidth={2} className="shrink-0 text-icon" />
        ...
        {result.flag_for_next && (
          <Field label="Flag for next shift">
            <div className="flex items-start gap-2 text-ink">
              <FlagIcon width={15} height={15} strokeWidth={2} className="mt-0.5 shrink-0 text-icon" />
